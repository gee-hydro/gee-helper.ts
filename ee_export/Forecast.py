...
import pandas as pd
import xarray as xr
from .dataset import PRCP_SOURCES
...
# WeatherNext2_mean: system:time_start为str
def bind_init_bounds(source):
    sample = base_col(source).first().get(source["init_field"]).getInfo()  # 判断类型
...
def forecast_col(source, t0, t1):
    ...
    col = (
        base_col(source)
        .filter(ee.Filter.gte(init, a))
        .filter(ee.Filter.lt(init, b))
...
PERIOD = {
    ...
    "month": (
        "MS",
        "%Y%m",
        lambda t: t + pd.offsets.MonthBegin(1),  # 推到下一个月的1号
        lambda t0, t1: N_INIT_DAY * (t1 - t0).days,
    ),
}


def export_period(source, t, fmt, t1_fn, n_ok_fn):
    t0, t1 = t, t1_fn(t)
    export_range(source, t0, t1, f"{t:{fmt}}", n_ok_fn(t0, t1))


def export_day(source, day):
    _, fmt, t1_fn, n_ok_fn = PERIOD["day"]
    export_period(source, day, fmt, t1_fn, n_ok_fn)


def export_month(source, month):
    _, fmt, t1_fn, n_ok_fn = PERIOD["month"]
    export_period(source, month, fmt, t1_fn, n_ok_fn)


def export_range(source, t0, t1, stamp, n_fc_ok):
